/* ===========================================================================
 * PetVetColorPicker — MS Office "More Colors"-style hexagon honeycomb picker.
 * Self-contained vanilla JS. Usage:
 *    PetVetColorPicker.init(containerEl, {
 *      value: '#93CAED',          // initial color
 *      onChangeLive: fn(hex),     // fired on drag/scrub
...
(function (global) {
  ...
  /* ---------------- honeycomb geometry ----------------
   * Pointy-top hexagons on axial (q,r) coords. Center cell = pure white.
   * Hue of each cell comes from its polar angle, so colors blend radially. */
  var MAX_RING = 3;
  var CELL_SIZE = 15; // hex radius (px)
  var DIRS = [[1, 0], [1, -1], [0, -1], [-1, 0], [-1, 1], [0, 1]];
  var RING_STYLE = {
    0: { s: 0, l: 100 },   // pure white center
    1: { s: 62, l: 60 },
    2: { s: 74, l: 53 },
    3: { s: 84, l: 47 }
  };
  ...
  function buildCells() {
    ...
    cells.forEach(function (c) {
      ...
      c.y = s * 1.5 * c.r;
      var ang = Math.atan2(c.x, -c.y) * 180 / Math.PI; // 0 deg = up
  ...
  function ensureStyles() {
    ...
    st.id = 'pvc-style';
    st.textContent = [
      '.pvc{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;color:#334155;width:100%;max-width:300px;}',
      '.pvc-top{font-size:11px;font-weight:700;letter-spacing:.03em;color:#64748b;text-transform:uppercase;padding:4px 2px 6px;}',
      '.pvc svg{display:block;width:100%;height:auto;margin:0 auto;touch-action:manipulation;}',
      ...
      '@media (prefers-reduced-motion:reduce){.pvc polygon{transition:none;}}'
    ].join('\n');
  ...
  function init(container, options) {
    ...
    svg.setAttribute('aria-label', 'Color palette');
    ...
    cells.forEach(function (c) {
      ...
      poly.setAttribute('data-h', c.hue);
